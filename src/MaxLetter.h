        ...
        while (!q.empty()) {
            ...
            FOR_ALPHA(i) {
                if (i != front && edge[front][i]) {
                    if (((int)word[front][i].back()->length()) + selfLength[i] + dp[front] > dp[i] && (dp[front] > 0 || outDegree[i] || selfLength[i])) {
                ...
                if (sccEdge[front][i]) {
                    for (auto& j : sccElement[front]) {
                        for (auto& k : sccElement[i]) {
                            if (edge[j][k] && maxOutLetter[j] + (int)word[j][k].back()->length() > maxInLetter[k] && (maxOutLetter[j] > 0 || outDegree[k] || edge[k][k])) {
